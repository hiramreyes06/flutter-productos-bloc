...
Widget _crearFondo( BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          ...
          Text('Luis Hiram', style: TextStyle( color: Colors.white, fontSize: 35.0 ) )
        ],
      )
    );
  }
    ...
    return SingleChildScrollView(
      child: Column(
        children: <Widget>[
         ...
         // Text('?Olvido la contraseña perro?'),
 ...
 Widget _crearPassword( LoginBloc bloc) {
  ...
  _login( BuildContext context, LoginBloc bloc ) async{
   ...
   final resp = await usuarioProvider.loginUsuario( bloc.emailValue, bloc.passwordValue );

    if( resp['ok'] ){
      Navigator.pushReplacementNamed(context, 'home');
    }else{

     mostrarAlerta(context, 'Datos incorrectos', 'Verifica tus datos ingresados');
    }
